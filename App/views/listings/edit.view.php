        ...
        <input type="hidden" name="_method" value="PUT">
            ...
            <?php if (isset($errors)) : ?>
                <?php foreach ($errors as $error) : ?>
                    ...
                    <div class="message bg-red-500 my-4 mx-2 text-white text-center rounded">
                        <?= $error ?>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
            <div class="mb-4">
                <input type="text" name="title" placeholder="Job Title (required)" class="w-full px-4 py-2 border rounded focus:outline-none" value='<?= $listing['title'] ?? ''?>'/>
            </div>
            <div class="mb-4">
                <textarea name="description" placeholder="Job Description (required)" class="w-full px-4 py-2 border rounded focus:outline-none"><?= $listing['description'] ?? ''?>'</textarea>
            </div>
            <div class="mb-4">
                <input type="text" name="salary" placeholder="Annual Salary (required)" class="w-full px-4 py-2 border rounded focus:outline-none" value='<?= $listing['salary'] ?? ''?>'/>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                <input type="text" name="city" placeholder="City (required)" class="w-full px-4 py-2 border rounded focus:outline-none" value='<?= $listing['city'] ?? ''?>'/>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                <input type="email" name="email" placeholder="Email Address For Applications (required)" class="w-full px-4 py-2 border rounded focus:outline-none" value='<?= $listing['email'] ?? ''?>'/>
            </div>
